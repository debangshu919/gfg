...
function ChatPanel({ activeChatId, onGraphRequest, onUpdateTitle, onSaveMessages, initialMessages, onReportData }) {
  ...
  const defaultMessages = initialMessages?.length > 0
    ? initialMessages
  ...
  const updateMessages = (updater) => {
    setMessages(prev => {
      ...
      onSaveMessages(activeChatId, next);
      return next;
    });
  };
    ...
    try {
      ...
      if (data.success) {
        ...
        const response = data.type === "data"
          ? (data.response || "Chart generated successfully")
        ...
        <div ref={menuRef} style={{ position: "relative" }}>
          ...
          {menuOpen && (
            <div style={{
              ...
              borderRadius: "6px", padding: "6px 0", width: "160px"
            }}>
              <button
